function [out_path out_name]= save_rat_data_imf(path_ratlogmat, path_ratout, path_savemat,ratNum,chNum,statType, imfNum)
...
for i = 1:numFiles
   ...
   strtemp=fileNames{i};
   fname = strcat(path_ratout,'/Rat',ratNum,'/EMD/IMF1h/','IMF',imfNum,'Rat',ratNum,'ch',chNum,'F',strtemp(1:4),'_',statType,'.bin');
   filenums(i)= str2num(strtemp(1:4));
   fid = fopen(fname,'rb');
   if (fid ~= -1)
...
cd(out_path)
out_name = strcat('IMF',imfNum,'DataRat',ratNum,'ch',chNum,'_',statType,'.mat');
% save(out_name,'data', 'filenums')
% save(out_name,'data_cell', 'data', 'filenums')
save(out_name,'data_cell', 'filenums')
cd (currdir)
