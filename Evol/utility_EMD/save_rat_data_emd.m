function [out_path out_name]= save_rat_data(path_ratlogmat, path_ratout, path_savemat,ratNum,chNum,statType, curr_imf, start_imf, end_imf)
...
for i = 1:numFiles
   strtemp=fileNames{i};
   fname = strcat(path_ratout,'/Rat',ratNum,'/EMD/','Rat',ratNum,'ch',chNum,'F',strtemp(1:4),'_',statType,'.bin');
   filenums(i)= str2num(strtemp(1:4));
   fid = fopen(fname,'rb');
   if (fid ~= -1)
       len = (end_imf - start_imf + 1)*2;
       fseek (fid, 0, 'bof');
       header = fread(fid, len,'uint64');
