...
function [A phi tbin_cent] = fit_sinusoids (t, x, period, fract_maxgap)
    ...
    for i=1:length(tbin_starts)
        ...
        elseif length(xtemp) < 5
            ...
            if plot_on; hold on; plot(ttemp, xtemp/normal_coef, ['k' plot_icon],'MarkerSize',12); end
            if plot_on;
...
%             phi(i)=mod(coefs(2),period);            % Fix this - find a better way, maybe put bounds on curve fitting
            ...
            phi = mod(phi,period);
            if normalize_data; normal_coef = abs(max(xtemp)); else normal_coef = 1; end
            if plot_on; hold on; ttemp2 = linspace(min(ttemp),max(ttemp),48); plot(ttemp2, cosfun(coefs,ttemp2)/normal_coef+mean(xtemp)/normal_coef,'r','LineWidth',2); end
        end
    end
    ...
    if plot_on
        if autoscale
            ...
            if normalize_data; axis([-0.01 1.05 -1.05 1.05]); end
            %set(gca,'YTickLabel','');
        end
    end
end
