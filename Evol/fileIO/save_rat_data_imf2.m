function [out_path out_name]= save_rat_data_imf2(path_ratlogmat, path_ratout, path_savemat,ratNum,chNum,statType, imfNum)
...
for i = 1:numFiles
   ...
   strtemp=fileNames{i};
   fname = strcat(path_ratout,'/Rat',ratNum,'/IMF',imfNum,'Rat',ratNum,'ch',chNum,'F',strtemp(1:4),'_',statType,'.bin');
   filenums(i)= str2num(strtemp(1:4));
   fid = fopen(fname,'rb');
   if (fid ~= -1)
